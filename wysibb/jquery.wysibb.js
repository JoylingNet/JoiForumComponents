...
//			direction:			"ltr",
			...
			buttons: 			"attimg,fontsize,fontfamily,|,bullist,numlist,|,justifyleft,justifycenter,justifyright,|,table,quote,code,-,bold,italic,underline,strike,sup,sub,fontcolor,removeFormat,|,img,link,at",
			allButtons: {
				bold : {
					title: CURLANG.bold,
        ...
        attimg : {
          type : 'hidden',
          transform : {
            '<img src="http://7xidaf.com1.z0.glb.clouddn.com/{SRC}" data-attach="{SRC}" />':"[attimg]{SRC}[/attimg]",
            '<img src="http://7xidaf.com1.z0.glb.clouddn.com/{SRC}" data-attach="{SRC}" width="{WIDTH}" height="{HEIGHT}"/>':"[attimg={WIDTH},{HEIGHT}]{SRC}[/attimg]"
          }
        },
				link : {
					title: CURLANG.link,
				...
				img : {
					...
					transform : {
						...
						'<img src="{SRC}" width="{WIDTH}" height="{HEIGHT}"/>':"[img={WIDTH},{HEIGHT}]{SRC}[/img]"
					}
				},
				bullist : {
        ...
        bgcolor: {
          type: "bgpicker",
          title: CURLANG.bgcolor,
          excmd: "backColor",
          valueBBname: "color",
          subInsert: true,
          transform: {
            '<span style="background-color:{COLOR}">{SELTEXT}</span>': '[backcolor={COLOR}]{SELTEXT}[/backcolor]',
            '<font style="background-color:{COLOR}">{SELTEXT}</font>': '[backcolor={COLOR}]{SELTEXT}[/backcolor]'
          }
        },
				table: {
					type: "table",
				...
				fontfamily: {
					...
					valueBBname: "font",
					options: [
            {title: "宋体",exvalue: "SimSun,宋体"},
            {title: "黑体",exvalue: "SimHei,黑体"},
            {title: "微软雅黑",exvalue: "'Microsoft Yahei',微软雅黑"},
            {title: "楷体,楷体_GB2312",exvalue: "SimKai,楷体,楷体_GB2312"},
            {title: "仿宋,仿宋_GB2312",exvalue: "仿宋,仿宋_GB2312"},
            {title: "幼圆",exvalue: "幼圆"},
            {title: "Arial",exvalue: "Arial"},
						{title: "Comic Sans MS",exvalue: "Comic Sans MS"},
						{title: "Courier New",exvalue: "Courier New"},
						{title: "Century Gothic",exvalue: "Century Gothic"},
						...
						tabs: [
							{
								title: '用户名',
								input: [
									{param: "NAME",title:'输入您要提及的用户名'}
								]
							}
						],
						onSubmit: function(cmd,opt,queryState) {
              var $obj = this.$modal.find('input[name="NAME"]');
              ...
              d.show($obj[0]);
              $.post('/user/at', {name: name}, function (data) {
                if (data.error) {
                  d.content(data.error);
                } else {
                  d.close();
                  that.insertAtCursor(that.getCodeByCommand(cmd, data));
                  ...
                  that.updateUI();
					...
					transform: {
						'<span data-at="{ID}" data-sign="{SIGN}">@{NAME}</span>':'[@={ID},{SIGN}]{NAME}[/@]'
					}
				},
				...
				fs_verybig: {
					...
					exvalue: "7",
					transform: {
						'<font size="7">{SELTEXT}</font>':'[size=7]{SELTEXT}[/size]'
					}
				},
		...
		buildToolbar: function() {
          ...
          switch (opt.type) {
            case "colorpicker":
              ...
              break;
            case "bgpicker":
              this.buildBgpicker($btnContainer,bn,opt);
              break;
            case "table":
    ...
    buildColorpicker: function(container,bn,opt) {
      ...
      joiColorPicker($.proxy(function (color) {
        this.selectLastRange();
        this.execCommand(bn, color == 'auto' ? basecolor : color);
      ...
      $btn.mousedown($.proxy(function (e) {
        e.preventDefault();
        this.dropdownclick(".wbb-cp", ".add-wrap", e);
      }, this));
      $btn.mousedown(function (e) {
        if (e.preventDefault) e.preventDefault();
      });
    },
    buildBgpicker: function(container,bn,opt) {
      var $btn = $('<div class="wysibb-toolbar-btn wbb-dropdown wbb-cp">').appendTo(container).append('<div class="ve-tlb-bgpick"></div><ins class="fonticon ar">\uE011</ins>').append(this.strf('<span class="btn-tooltip">{title}<ins/></span>', {title: opt.title}));
      var $bg = $btn.find(".ve-tlb-bgpick");

      var $dropblock = $('<div class="add-wrap">').appendTo($btn);
      ...
      $btn.bind('queryState', $.proxy(function (e) {
        //queryState
        $bg.css("background-color", basecolor);
        var r = this.queryState(bn, true);
        if (r) {
          $bg.css("background-color", (this.options.bbmode) ? r.color : r);
        }
      }, this));
      $btn.mousedown($.proxy(function (e) {
        e.preventDefault();
      ...
      $btn.mousedown(function (e) {
		...
		buildTablepicker: function(container,bn,opt) {
			...
			var $listblock = $('<div class="add-wrap">').appendTo($btn);
			...
			$btn.mousedown($.proxy(function(e) {
				e.preventDefault();
				this.dropdownclick(".wbb-tbl",".add-wrap",e);
			},this));
		...
		buildSelect: function(container,bn,opt) {
			...
			var $sblock = $('<div class="add-wrap">').appendTo($btn);
			...
			$btn.mousedown($.proxy(function(e) {
				e.preventDefault();
				this.dropdownclick(".wbb-select",".add-wrap",e);
			},this));
		...
		buildSmilebox: function(container,bn,opt) {
			if (this.options.smileList && this.options.smileList.length>0) {
				...
				$btn.mousedown($.proxy(function(e) {
					e.preventDefault();
					this.dropdownclick(".wbb-smilebox",".add-wrap",e);
				},this));
	...
	$.fn.wdrag = function(opt) {
		...
		opt.scope.drag_mousemove = function(e) {
          ...
          opt.scope.$toolbar.css("max-height",nheight+'px');
        }
			}
		};
